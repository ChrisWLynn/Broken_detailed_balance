...
% written for the gambling HCP data

% Colormap for flux map:
color = .8*(.8*[0 77 128]/255) + .2*[1 1 1];
X = linspace(1, 0, 1000)';
cmap = X*[1 1 1] + (1-X)*color;

% Increase scale of current:
scale = 2000; % For gambling
% scale = 10000; % For gambling (shuffle)
current_temp = scale*current;

% Units to measure current (in inverse seconds):
units = 10^(-3); % For gambling
% units = 10^(-4); % For gambling (shuffle)
...
lim = caxis;
% caxis([0 max(lim)])
caxis([0 .045]) % For comp with rest
